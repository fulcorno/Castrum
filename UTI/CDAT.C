...
static void near loadmenu(void)
{
	...
	if (strcmp(rec[0],"0")) {
		...
		m++;
		if(debug) printf("\tF%2d: icona %d, azione %d, next %d; %s\n",
...
/* blocco ricorrente: da ^K-C-opiare quando si aggiunge un nuovo .DCA
static void near loadxxx(void)
{
    indice_xxx i ;
    ... variabili locali
    xxx = calloc(max_xxx,sizeof(t_xxx)) ;
    tot_xxxs=-1; (non sempre, ma spesso)
    while (getrecord()) {
	if (strcmp(rec[0],"0")) {  --- per convenzione una riga isolata "0"
					indica la fine del file
	    ... qui metto i dati a posto leggendoli da rec[0..]
...
void main(int argc, char ** argv)
{
	...
	/*
	if(match(name,"P_SPOSTA") ) loadp_sposta() ;
	if(match(name,"P_AZIONE") ) loadp_azione() ;
	if(match(name,"P_POSIZ")  ) loadp_posiz() ;
	if(match(name,"P_MILLE")  ) loadp_mille() ;
	if(match(name,"Q_INDO")   ) loadq_indo() ;
	if(match(name,"Q_FOLLE")  ) loadq_folle() ;
	*/
    }
}
