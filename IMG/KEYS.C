...
#define SUPPRESSED { {1,0,1}, {0,1,0}, {1,0,1} }
...
void buttons(void)
{
	...
	putimage(x-SHADE,y, ombra, AND_PUT) ;
	putimage(x,y, tasto, COPY_PUT) ;
    }
    setfillpattern(fill[FILL_100],1) ;
    setcolor(0) ;
    for(i=0; i<COLS; ++i) for(j=ROWS-1; j>=0; --j)
	...
	x = OFS_X+i*DIST_X-EXT_X/2 ;
	y = OFS_Y+j*DIST_Y-EXT_Y/2 ;
	bar(x+X5, y+Y5, x+X8, y+Y8) ;
	line(x+X6,y+Y6, x+X8,y+Y8) ;
    }
}
