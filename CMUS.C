...
void getparameters(int argc, char** argv)
{
	...
	if ((argc==2)&&(!strcmp(argv[1],"?"))) {
		/*DisplayHelp() ;*/
		exit(255); /* l"errore ä il + grave: l"utente non sa usare il comando */
	}
...
void outnota(char *s)
{
	...
	if (!strcmp(s,"'")) what=stacca ;
		...
		if (s[0]=='?') {
			...
			questa=ch2note(c) ;
			if ((s[0]!='*')&&(altera==0)) {				if (belongs(questa,diesis)) altera=1 ;				else if (belongs(questa,bemolli)) altera=-1 ;
			}
...
void main(int argc, char ** argv)
{
	...
	while (!feof(inp)) {
		...
		while (*frase) {			if (frase[0]=='!') {
				...
				if (*parola)					if (strchr("=%&@",parola[0])) controllo(parola) ;					else outnota(parola) ;			}			strcpy(frase,frase+strlen(parola)+1) ;
		}
